...
void MainWindow::update_activeWindow(){
    ...
    for (int i = 0; i < windows.size(); i++){
        ...
        QAction *action  = this->ui->menuActive_Window->addAction(text);
...
void MainWindow::on_actionLoad_triggered()
{
    ...
    QStringList fileName = QFileDialog::getOpenFileNames(this, tr("Load Module"), tr(".RTUdata/module/"), tr("(*.dbe)"));
...
//            QApplication::processEvents();
            module_name[module_count] = work->loadModule(modelTree, this->ui->treeView, pth, newName);
...
void MainWindow::on_treeView_clicked(const QModelIndex &index)
{
    ...
    if(work->checkIfmodule(name)) {
        ...
        module_address_sv = ".RTUdata/module/" + module_name_sv;
    } else {
        module_name_sv = "";
        module_address_sv = "";
        return;
...
void MainWindow::Refresh_Tree()
{
    ...
    for(int i = 0; i < files.count(); i++){
        ...
        if(moduleName != "none"){
            filePath.sprintf(".RTUdata/module/%s", moduleName.toUtf8().data());
